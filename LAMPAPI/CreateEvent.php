<?php
    ...
    $time = $inData['time'];
    $locId = $inData['locId']; // Assuming the JSON data has a 'locId' field
    ...
    if ($conn->connect_error) {
        ...
        $checkLocationStmt = $conn->prepare("SELECT COUNT(*) FROM Locations WHERE LocID = ?");
        $checkLocationStmt->bind_param("s", $locId);
        $checkLocationStmt->execute();
        $checkLocationStmt->bind_result($locationCount);
        ...
        if ($locationCount > 0) {
            ...
            returnWithError("Invalid location ID: $locId");
        }
